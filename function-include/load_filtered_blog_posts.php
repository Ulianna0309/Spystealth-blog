...
function load_filter_blog_posts() {
    ...
    if( $query->have_posts() ):
        ...
        $query->the_post();
        if ($count == 1 || $count == 2) {
            $class_col = ' article--col-2';
        } else {
            $class_col = ' article--col-3';
        }
        ?>
        <article class="article post-card-animation post-card-animation<?= $class_col; ?>">
            <?php
            ...
            <div class="article__content">
                <div class="article__header">
                    ...
                    ?>
                    <a href="<?= $category_link?>" class="category-label category-label--<?= $category[0]->slug; ?>">
                        <?= $category_name?>
                    </a>
...
add_action('wp_ajax_load-filter-blog-posts', 'load_filter_blog_posts');
add_action('wp_ajax_nopriv_load-filter-blog-posts', 'load_filter_blog_posts');
