...
function load_more_all_posts(){
    ...
    if( $query->have_posts() ):
        while($query->have_posts()): $query->the_post();
            $count == 1 || $count == 6
                ? $class_col = ' article--col-3-4'
                : $class_col = ' article--col-3';
            ?>
            <article class="article post-card-animation<?= $class_col; ?>">
                <?php
...
add_action('wp_ajax_loadmore-home', 'load_more_all_posts');
add_action('wp_ajax_nopriv_loadmore-home', 'load_more_all_posts');
