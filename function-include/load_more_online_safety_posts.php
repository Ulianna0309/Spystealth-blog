...
function load_more_online_safety_posts() {
    ...
    if( $query->have_posts() ):
        while($query->have_posts()): $query->the_post();
            if ($count == 1 || $count == 10) {
                $article_class = ' article--col-2';
                $article_content_class = ' article__content--fix-height';
            } else {
                $article_content_class = '';
                $article_class = ' article--col-4';
            }
            ?>
            <article class="article post-card-animation article-online-safety<?= $article_class; ?>">
                <?php
                ...
                ?>
                <div class="article__content<?= $article_content_class; ?>">
                    <div class="article__header">
                        <span class="category-label category-label--online-safety category-label--no-changes">online safety</span>
...
add_action('wp_ajax_loadmore-online-safety', 'load_more_online_safety_posts');
add_action('wp_ajax_nopriv_loadmore-online-safety', 'load_more_online_safety_posts');
